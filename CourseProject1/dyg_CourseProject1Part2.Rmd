...
##Conclusions
Starting from the most significant p-value, we can see that every level of dose, when compared to each other level is statistically significant - that is
to say that we reject the Null Hypothesis (that the means of each group are
equal) for the Alternate Hypothesis (that the means of each group are not 
equal) for all but the last three rows in the table. In sum, Dose is a 
statistically significant variable in this experiment when convolved with the 
